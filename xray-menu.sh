...
add-ws() {
    ...
    echo ""
    read -p "Username  : " user
    if grep -qw "$user" /etc/diyvpn/xray/user.txt; then
        echo -e ""
    ...
    none=80
    email=${user}
    cat >/etc/diyvpn/xray/$user-tls.json <<EOF
      {
       "v": "2",
       "ps": "${user}",
       "add": "${domain}",
...
EOF
    ...
    cat >/etc/diyvpn/xray/$user-none.json <<EOF
      {
      "v": "2",
      "ps": "${user}",
      "add": "${domain}",
...
EOF
    echo -e "${user}\t${uuid}\t${exp}" >>/etc/diyvpn/xray/user.txt
    cat /etc/diyvpn/xray/conf/05_VMess_WS_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","alterId": 0,"add": "'${domain}'","email": "'${email}'"}]' >/etc/diyvpn/xray/conf/05_VMess_WS_inbounds_tmp.json
...
},{"id": "'""$uuid""'","alterId": '"0"',"email": "'""$email""'"' /etc/diyvpn/xray/conf/vmess-nontls.json
    vmess_base641=$(base64 -w 0 <<<$vmess_json1)
    vmess_base642=$(base64 -w 0 <<<$vmess_json2)
    vmesslink1="vmess://$(base64 -w 0 /etc/diyvpn/xray/$user-tls.json)"
    vmesslink2="vmess://$(base64 -w 0 /etc/diyvpn/xray/$user-none.json)"
    cat <<EOF >>"/etc/diyvpn/config-user/${user}"
${vmesslink1}
${vmesslink2}
EOF
    base64Result=$(base64 -w 0 /etc/diyvpn/config-user/${user})
    echo ${base64Result} >"/etc/diyvpn/config-url/${uuid}"
...
add-vless() {
    ...
    echo -e "${user}\t${uuid}\t${exp}" >>/etc/diyvpn/xray/user.txt
    sed -i '/#xray$/a\### '"$user $exp"'\
},{"id": "'""$uuid""'","email": "'""$email""'"' /etc/diyvpn/xray/vless-nontls.json
    cat /etc/diyvpn/xray/conf/03_VLESS_WS_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","email": "'${email}'"}]' >/etc/diyvpn/xray/conf/03_VLESS_WS_inbounds_tmp.json
...
add-trojan() {
    ...
    email=${user}
    echo -e "${user}\t${uuid}\t${exp}" >>/etc/diyvpn/xray/user.txt
    cat /etc/diyvpn/xray/conf/04_trojan_TCP_inbounds.json | jq '.inbounds[0].settings.clients += [{"password": "'${uuid}'","email": "'${email}'"}]' >/etc/diyvpn/xray/conf/04_trojan_TCP_inbounds_tmp.json
...
add-xtls() {
    ...
    email=${user}
    echo -e "${user}\t${uuid}\t${exp}" >>/etc/diyvpn/xray/user.txt
    cat /etc/diyvpn/xray/conf/02_VLESS_TCP_inbounds.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","add": "'${domain}'","flow": "xtls-rprx-direct","email": "'${email}'"}]' >/etc/diyvpn/xray/conf/02_VLESS_TCP_inbounds_tmp.json
...
del-user() {
    ...
    echo -e "\033[0;31m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[33m"
    cat /etc/diyvpn/xray/user.txt | awk '{print $1}' | sort | uniq
    echo -e ""
    read -p "Username : " user
    echo -e ""
    if ! grep -qw "$user" /etc/diyvpn/xray/user.txt; then
    ...
    exp="$(cat /etc/diyvpn/xray/user.txt | grep -w "$user" | awk '{print $3}')"
...
extend_user() {
    ...
    if ! grep -qw "$user" /etc/diyvpn/xray/user.txt; then
        ...
        uuid=$(grep -wE "$user" "/etc/diyvpn/xray/user.txt" | awk '{print $2}')
        exp=$(grep -wE "$user" "/etc/diyvpn/xray/user.txt" | awk '{print $3}')
        echo "$user : $exp"
        ...
        sed -i "/$user/d" /etc/diyvpn/xray/user.txt
        echo -e "${user}\t${uuid}\t${exp4}" >>/etc/diyvpn/xray/user.txt
...
END
    ...
    bash <(curl -sSL https://raw.githubusercontent.com/razerhunters/multi/main/ceknet.sh)
...
change-domain() {
    ...
    rm /etc/diyvpn/xray/domain
    echo "$host" >>/etc/diyvpn/xray/domain
    echo "$host" >/root/domain
    domain=$(cat /etc/diyvpn/xray/domain)
    ...
    sleep 2
    /root/.acme.sh/acme.sh --issue -d $domain --standalone -k ec-256 --server letsencrypt >>/etc/diyvpn/tls/$domain.log
    ~/.acme.sh/acme.sh --installcert -d $domain --fullchainpath /etc/diyvpn/xray/xray.crt --keypath /etc/diyvpn/xray/xray.key --ecc
    cat /etc/diyvpn/tls/$domain.log
...
change_port() {
    ...
    if [[ -z $cek ]]; then
        sed -i "s/$multi/$multi2/g" /etc/diyvpn/xray/conf/02_VLESS_TCP_inbounds.json
...
check_login() {
    ...
    data=($(cat /etc/diyvpn/xray/user.txt | awk '{print $1}' | sort | uniq))
...
backup_vps() {
    ...
    cp /etc/diyvpn/xray/user.txt backup/
    cp -r /etc/diyvpn/xray/conf backup/conf
    ...
    LLatest=$(date)
    Get_Data() {
        git clone https://github.com/razerhunters/user-backup-db.git /root/user-backup/ &>/dev/null
    }
...
" >>/root/user-backup/$Name/$Name-last-backup
    ...
    Save_And_Exit() {
        cd /root/user-backup
        git config --global user.email "diyvpn@gmail.com" &>/dev/null
        git config --global user.name "razerhunters" &>/dev/null
        rm -rf .git &>/dev/null
        git init &>/dev/null
        git add . &>/dev/null
        git commit -m m &>/dev/null
        git branch -M main &>/dev/null
        git remote add origin https://github.com/razerhunters/user-backup-db
        git push -f https://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@github.com/razerhunters/user-backup-db.git &>/dev/null
    }
    ...
    link="https://raw.githubusercontent.com/xxdiyzvpn/LITE//main/$Name/$Name.zip"
    sleep 1
...
restore_vps() {
    ...
    sleep 1
    cp /root/backup/user.txt /etc/diyvpn/xray/ &>/dev/null
    echo -e "[ ${green}INFO${NC} ] • Restoring admin data..."
    sleep 1
    cp -r /root/backup/conf /etc/diyvpn/xray &>/dev/null
    rm -rf /root/backup &>/dev/null
...
menu_sts() {
    chck_pid
    if [[ ! -z "${PID}" ]]; then
        echo -e "Status  : ${Green_font_prefix} Xray-Core [ON]${Font_color_suffix} | ${Green_font_prefix}Nhinx [ON]${Font_color_suffix}"
    else
        echo -e "Status  : ${Green_font_prefix} Xray-Core [OFF] ${Font_color_suffix} | ${Red_font_prefix}Nginx [OFF] ${Font_color_suffix}"
    fi
}
clear
echo -e ""
...
echo -e "\033[0;33m ░█──▀█ ░█▄▄▄█ ░█─░█ ─▀▄▄▀ ░█▄▄█ ░█▄▄▄ ░█▄▄▄█ ░█▄▄▄█ ▄▀░▀▄"
echo -e ""
echo -e "\033[1;32m Premium Script DIYNETWORKK\033[0m"                                       
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
echo -e "\E[41;1;30m                 • INFORMATION •                  \E[33m"
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
echo -e "\e[0;36m IP VPS Number       : $IPVPS"
echo -e "\e[0;36m Telegram Own        : @DIYNETWORKK"
echo -e "\e[0;36m Script Version      : 5.0 (Latest)"
echo -e "\e[0;36m ISP/Provider Name   : $ISP "
echo -e "\e[0;36m Client Name         : $name"
echo -e "\e[0;36m Validity Script     : $exp"
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
echo -e "\E[41;1;30m                 • SSH  MENU •                    \E[33m"
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
...
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
echo -e "\E[41;1;30m                 • XRAY MENU •                    \E[33m"
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
echo -e " [\e[35m•7\e[34m ] Add XRay VMess     [\e[35m•11\e[34m] Add XRay VLess"
echo -e " [\e[35m•8\e[34m ] Delete XRay VMess  [\e[35m•12\e[34m] Delete XRay VLess"
echo -e " [\e[35m•9\e[34m ] Extend XRay Vmess  [\e[35m•13\e[34m] Extend XRay VLess"
echo -e " [\e[35m•10\e[34m] Check User Login   [\e[35m•14\e[34m] Check User Login"
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
echo -e " [\e[35m•15\e[32m] Add XRay XTLS      [\e[35m•19\e[32m] Add XRay Trojan"
echo -e " [\e[35m•16\e[32m] Delete XRay XTLS   [\e[35m•20\e[32m] Delete XRay TRojan"
echo -e " [\e[35m•17\e[32m] Extend XRay XTLS   [\e[35m•21\e[32m] Extend XRay TRojan"
echo -e " [\e[35m•18\e[32m] Check User Login   [\e[35m•22\e[32m] Check User Login"
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
echo -e "\E[41;1;30m                 • SETTING MENU •                 \E[33m"
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
echo -e " [\e[35m•23\e[39m] Setup DNS          [\e[35m•28\e[39m] Check All Port"
echo -e " [\e[35m•24\e[39m] Check DNS Region   [\e[35m•29\e[39m] Backup"
echo -e " [\e[35m•25\e[39m] Change VPS Domain  [\e[35m•30\e[39m] Restore"
echo -e " [\e[35m•26\e[39m] Change Port        [\e[35m•31\e[39m] BBR Menu"
echo -e " [\e[35m•27\e[39m] Restart Service    [\e[35m•32\e[39m] Usage Server"
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
menu_sts
echo -e "\033[0;31m──────────────────────────────────────────────────\033[33m"
