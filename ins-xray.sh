...
# INSTALL XRAY
wget -c -P /etc/diyvpn/xray/ "https://github.com/XTLS/Xray-core/releases/download/v1.4.5/Xray-linux-64.zip"
unzip -o /etc/diyvpn/xray/Xray-linux-64.zip -d /etc/diyvpn/xray
rm -rf /etc/diyvpn/xray/Xray-linux-64.zip
chmod 655 /etc/diyvpn/xray/xray
# XRay boot service
...
NoNewPrivileges=yes
ExecStart=/etc/diyvpn/xray/xray run -confdir /etc/diyvpn/xray/conf
Restart=on-failure
...
CapabilityBoundingSet=CAP_NET_BIND_SERVICE CAP_NET_RAW
NoNewPrivileges=yes
ExecStart=/etc/diyvpn/xray/xray run -config /etc/diyvpn/xray/vless-nontls.json
Restart=on-failure
RestartPreventExitStatus=23
...
rm -rf /etc/diyvpn/xray/conf/*
cat <<EOF >/etc/diyvpn/xray/conf/00_log.json
{
  "log": {
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/02_VLESS_TCP_inbounds.json
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
  ...
  "inbounds": [
    {
      ...
      "streamSettings": {
        ...
        "xtlsSettings": {
          ...
          "certificates": [
            {
              "certificateFile": "/etc/diyvpn/xray/xray.crt",
              "keyFile": "/etc/diyvpn/xray/xray.key",
              "ocspStapling": 3600,
              "usage": "encipherment"
            }
          ]
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/03_VLESS_WS_inbounds.json
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/04_trojan_gRPC_inbounds.json
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/04_trojan_TCP_inbounds.json
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/10_trojan_XTLS_inbounds.json
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/05_VMess_WS_inbounds.json
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
...
EOF
cat <<EOF >/etc/diyvpn/xray/conf/06_VLESS_gRPC_inbounds.json
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
...
cat >/etc/diyvpn/xray/conf/vmess-nontls.json <<END
{
  "log": {
    "access": "/var/log/xray/access.log",
    "error": "/var/log/xray/error.log",
...
cat >/etc/diyvpn/xray/vless-nontls.json <<END
{
  "log": {
    "access": "/var/log/xray/access.log",
